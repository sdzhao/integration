...
## direct = direct effect or not
igwas <- function(Y,G,S,X=NULL,family, direct = FALSE)
{
    ...
    if (!direct) {
        ...
        ## fit outcome model and transcript model
        if(r==0){
            fit.o <- glm(Y~G+S,family=family);
        } else {
            fit.o <- glm(Y~G+S+X,family=family);
        }
        ystar <- G%*%coef(fit.o)[2:(1+p)];
        if(r==0){
            fit.t <- lm(ystar~S);
        ...
        B <- t(covs)%*%G/n;
        B <- cbind(rep(0,nrow(B)),B,
                   matrix(0,nrow=nrow(B),ncol=q+r));
        Cinv <- solve(t(covs)%*%covs/n);
        ee <- cbind(estfun(fit.o),estfun(fit.t));
        V <- t(ee)%*%ee/n;
        V11 <- V[1:(1+p+q+r),1:(1+p+q+r)];
        V12 <- V[1:(1+p+q+r),(2+p+q+r):ncol(V)];
        V21 <- V[(2+p+q+r):ncol(V),1:(1+p+q+r)];
        V22 <- V[(2+p+q+r):ncol(V),(2+p+q+r):ncol(V)];
...
## family = type of outcome; same as glm argument
## direct = direct effect or not
igwas.cc <- function(Y,P,G,S,X=NULL,family, direct = FALSE)
{
    ## make everything into a matrix
    Y <- as.matrix(Y); G <- as.matrix(G); S <- as.matrix(S);
    if(is.null(X[1]))
