## int.R
## simulation for case with interactions
...
## 1. nonlinear in genes and clinical covs
## 2. correct gene set
## 3. no direct effect
## 4. no measurement error
aG <- sample(c(-1,1),p,replace=TRUE)*runif(p,0.05,0.7);
aX <- rnorm(r);
## fewer interactions with lower magnitudes
AGS <- matrix(sample(0:1,p*q,replace=TRUE,prob=c(0.9,0.1)),
              nrow=p,ncol=q);
AGS[AGS==1] <- sample(c(-1,1),sum(AGS==1),replace=TRUE)*
  runif(sum(AGS==1),0.05,0.5);
AGS <- cbind(AGS,0); ## null SNP involved in no interactions
AGX <- matrix(sample(0:1,p*r,replace=TRUE,prob=c(0.7,0.3)),
              nrow=p,ncol=r);
AGX[AGX==1] <- sample(c(-1,1),sum(AGX==1),replace=TRUE)*
  runif(sum(AGX==1),0.05,0.5);
ASX <- matrix(sample(0:1,q*r,replace=TRUE,prob=c(0.9,0.1)),
              nrow=q,ncol=r);
ASX[ASX==1] <- sample(c(-1,1),sum(ASX==1),replace=TRUE)*
  runif(sum(ASX==1),0.05,0.5);
ASX <- rbind(ASX,0); ## null SNP involved in no interactions
...
Y <- G%*%aG+X%*%aX+
    diag(G%*%AGS%*%t(S))+diag(G%*%AGX%*%t(X))+diag(S%*%ASX%*%t(X))+
    rnorm(n,0,2);
...
save.image(paste("results/int",env,".RData",sep=""));
