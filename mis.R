## mis.R
## simulation for misspecified gene set
...
## outcome model with misspecified gene set
## 1. linear in genes and clinical covs
## 2. misspecified gene set
## 3. no direct effect
...
## keep the first p/2 genes. generate additional p/2 genes to be
## false positives included in the gene set. keep the structure
## of the first p genes the same as above
set.seed(1); ## need seed or Sigma.all is not necessarily PSD
newSigma <- cor(matrix(rnorm(p*1.5*10),nrow=10));
newSigma[1:p,1:p] <- Sigma; ## same corr as previous sims
newGamS <- matrix(sample(0:1,p/2*(q+1),replace=TRUE,prob=c(0.5,0.5)),
               nrow=q+1,ncol=p/2);
newGamS[newGamS==1] <- sample(c(-1,1),sum(newGamS==1),replace=TRUE)*
  runif(sum(newGamS==1),0.05,1);
newGamS <- cbind(GamS,newGamS);
newGamX <- matrix(sample(0:1,r*p/2,replace=TRUE,prob=c(0.5,0.5)),
                 nrow=r,ncol=p/2);
newGamX[newGamX==1] <- sample(c(-1,1),sum(newGamX==1),replace=TRUE)*
  runif(sum(newGamX==1),0.05,1);
newGamX <- cbind(GamX,newGamX);
newaG <- c(aG,rep(0,p/2)); ## new p/2 genes don't affect outcome
...
G <- S%*%newGamS+X%*%newGamX+mvrnorm(n,rep(0,p*1.5),sig2*newSigma);
Y <- G%*%newaG+X%*%aX+rnorm(n,0,2);

## for the analysis, use only the first (important) and last (not
## important) p/2 genes
G <- G[,c(1:(p/2),(p+1):ncol(G))];
...
G <- S%*%newGamS+X%*%newGamX+mvrnorm(nn,rep(0,p*1.5),sig2*newSigma);
Y <- sapply(expit(-3+G%*%newaG+X%*%aX),function(x)
...
save.image(paste("results/mis",env,".RData",sep=""));
