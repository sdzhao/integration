...
## outcome model with measurement error
...
## save random seed to be consistent with easy.R
seed <- .Random.seed;
## add measurement error with var=2
G <- G+t(replicate(n,arima.sim(list(order=c(1,0,0),ar=ar),n=p)))*
    sqrt(2)*sqrt(1-ar^2);
...
Y <- sapply(expit(-3+G%*%aG+X%*%aX),function(x)
            { sample(0:1,1,prob=c(1-x,x)); });
## add measurement error
G <- G+t(replicate(nn,arima.sim(list(order=c(1,0,0),ar=ar),n=p)))*
    sqrt(2)*sqrt(1-ar^2);
...
save.image(paste("results/measerr",env,".RData",sep=""));
